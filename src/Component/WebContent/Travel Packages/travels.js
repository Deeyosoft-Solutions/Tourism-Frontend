...
const Travels = () => {
  ...
  // Filter state - ADD THE MISSING FIELDS
  ...
  // Fetch travel packages WITH PARAMS
  const { data: packagesRaw, isLoading, error } = useGetTravelPackagesQuery(queryParams);
  ...
  const updateItemsPerPage = () => {
    ...
    if (width >= 1024) setItemsPerPage(9);
  ...
  const totalPages = packagesRaw?.total 
    ? Math.ceil(packagesRaw.total / itemsPerPage)
  ...
  if (error)
    return (
      <ErrorMessage message="Failed to load travel packages." className="m-4" />
    );
        ...
        <div className={`grid gap-3 ${gridColumnsClass}`}>
          {packages.length === 0 ? (
            <div className="col-span-full text-center py-12 text-gray-500">
              No travel packages found matching your filters.
            </div>
          ) : (
            packages.map((pkg) => (
              <div
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-600 mb-4">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span>
                        {pkg.destinations?.[0]?.name || "Destination"}
                      </span>
                    </div>
                    <div className="flex items-center gap-1">
                      <svg
